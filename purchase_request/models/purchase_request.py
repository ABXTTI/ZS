...
class PurchaseRequest(models.Model):
    ...
    def _compute_is_editable(self):
        for rec in self:
            if rec.state in ("to_approve", "approved1", "approved", "rejected", "done"):
    ...
    approved_by1 = fields.Many2one('res.users', string="First Approval by", compute="button_approved1", store=True)
    approved_by2 = fields.Many2one('res.users', string="Second Approval by", compute="button_approved", store=True)
    ...
    def button_approved1(self):
        print("#############################################", self.env.uid)
        self.approved_by1 = self.env.uid
        return self.write({"state": "approved1"})

    def button_approved(self):
        print("#############################################", self.env.uid)
        self.approved_by2 = self.env.uid
