    ...
    def check_report(self, data):
        ...
        if data['report_type'] == 'config':
            ...
            if data['account_report_id'][0] == 'journals_audit':
                ...
                )._get_accounts_dynamic(accounts, display_account, used_context)
...
class ReportTrialBalanceExt(models.AbstractModel):
    ...
    def _get_accounts_dynamic(self, accounts, display_account, used_context):
        ...
        account_res = []
        for account in accounts:
            res = dict((fn, 0.0) for fn in ['credit', 'debit', 'balance','closing_balance', 'initial_balance'])
            ...
            initial_balance_list = self.env['account.move.line'].search([('account_id', '=', account.id),
                                                                         ('parent_state', '=', 'posted'),
                                                                         ('date', '<', used_context['date_from'])
                                                                         ]).sorted(lambda o: o.date).mapped(lambda o: o.debit or -o.credit)

            if account.id in account_result:
                res['debit'] = account_result[account.id].get('debit')
                res['credit'] = account_result[account.id].get('credit')
                res['balance'] = account_result[account.id].get('balance')
                res['initial_balance'] = sum(initial_balance_list)
                res['closing_balance'] = (res['initial_balance'] + res['balance']) or 0

            elif initial_balance_list:
                res['debit'] = 0
                res['credit'] = 0
                res['balance'] = 0
                res['initial_balance'] = sum(initial_balance_list)
                res['closing_balance'] = (res['initial_balance'] + res['balance'])

            if display_account == 'all' and not currency.is_zero(res['closing_balance']):
