    ...
    def get_absents_etc(self):
        ...
        if self.employee_id:
            ...
            absent_days = days.search([('employee_id', '=', self.employee_id.id),
                                                        ...
                                                        ('x_day', '<=', self.date_to),
                                                        ('state', '=', 0)])
            leaves = days.search([('employee_id', '=', self.employee_id.id),
                                  ('x_day', '>=', self.date_from),
                                  ('x_day', '<=', self.date_to),
                                  ('state', '=', 6)])
            ...
            sum_leaves = sum(rec.days_count for rec in leaves if rec.days_count)
            # print(sum_overtime, "$%%###########")
            ...
            var.append((0, 0, {
                ...
                'number_of_days': 0,
                'number_of_hours': 0,
                'amount': sum_leaves if sum_leaves else 0.0,
                'contract_id': self.contract_id.id,
            }))
        ...
        #//////////////////////////////////Added by AB///////////////////////////////////////////
            for p in payslips:
                if p.employee_id.id == employee.id:
                    ...
                    leaves = days.search([('employee_id', '=', p.employee_id.id),
                                          ('x_day', '>=', p.date_from),
                                          ('x_day', '<=', p.date_to),
                                          ('state', '=', 6)])
