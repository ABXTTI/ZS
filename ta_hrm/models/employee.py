...
class HrAttendance(models.Model):
    ...
    def get_overtime(self):
        for rec in self:
            ...
            if rec.worked_hours > rec.shift_hours:
                rec.t_overtime = rec.worked_hours - rec.shift_hours
                rec.rule_overtime = rec.t_overtime / 2
                rec.x_workedhours = rec.worked_hours - rec.rule_overtime
                rec.x_checkout = rec.check_out - datetime.timedelta(hours=rec.rule_overtime)
            else:
                rec.t_overtime = 0.0
                rec.rule_overtime = 0.0
                rec.x_workedhours = rec.worked_hours
    ...
    x_checkout = fields.Datetime(string="Check_Out.")
    x_workedhours = fields.Float(string="Worked Hours")
